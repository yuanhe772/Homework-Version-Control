 ...
 * Calculate the delta V and Hessian Matrix according to either Quasi-newton method
 * or Secant method.
 */
public class gradientEsti {

	/* Class Variant: the perturbation proportion*/
	static final double perturb = 0.0001;
//	static final double perturb = 0.0000001;
	 ...
	 * @throws Exception */
	public static Vector delQN(Vector para, int norm) throws Exception {
		...
		for(int i=0; i<para.len; i++) {
			...
			delVQN.v[i] = (iterativeSolver.V(onePerturbed1, norm) - iterativeSolver.V(onePerturbed2, norm)) / (allPerturbed.v[i]*2);
		}
		return delVQN;
	}
	 ...
	 * @throws Exception */
	public static FullMatrix hessianQN(Vector para, int norm) throws Exception {
		// Perturb the parameter with scalar = perturb
		Vector allPerturbed = para.scale(perturb);
		...
		for(int i=0; i<para.len; i++) {
			for(int j=0; j<para.len; j++) {
				...
				HessVQN.full[i][j] = (iterativeSolver.V(twoPerturbed1, norm) - iterativeSolver.V(twoPerturbed2, norm) 
						- iterativeSolver.V(twoPerturbed3, norm) + iterativeSolver.V(twoPerturbed4, norm)) /
	 ...
	 * Return: Vector delVQN
	 * @throws Exception */
	public static Vector delSC(Vector para, int norm) throws Exception {
		// Perturb the parameter with scalar = perturb
		Vector allPerturbed = para.scale(perturb);
		...
		// The numerator
		double numerator = iterativeSolver.V(perturbed1, norm) - iterativeSolver.V(perturbed2, norm);
	 ...
	 * Return: FullMatrix HessVQN
	 * @throws Exception */
	public static FullMatrix hessianSC(Vector para, int norm) throws Exception {
		// Perturb the parameter with scalar = perturb
		Vector allPerturbed = para.scale(perturb);
		...
		// The numerator
		double numerator = iterativeSolver.V(perturbed1, norm) - iterativeSolver.V(para, norm) + iterativeSolver.V(perturbed2, norm);
