...
public class iterativeSolver {
	...
	public static double[] iterSolver(int estType, int normType, int modelType, Vector paraGuess, double tolerance) 
		...
		while(V(currPara, normType) > Math.pow(10, -7) && increMag(deltaPara, currPara) > tolerance) {
			...
			System.out.println((MODEL==POW ? "Power Law - ":"EKV - ") + (estType==1?"Quasi-Newton":"Secant") +
					" method's "+count + "th iteration:\n    [Is, Kapa, Vth] = " + Arrays.toString(currPara.v) + 
			...
			if(count > 20000) {
				double convResult[] = {0, quadConv(preprevPara, prevPara, currPara, normType), V(currPara, normType), count};
				return convResult;
			}
		}

		// Return the result
		double convResult[] = {1, quadConv(preprevPara, prevPara, currPara, normType), V(currPara, normType), count};
		return convResult;
	}
