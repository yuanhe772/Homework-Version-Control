...
public class SparseMatrix {
	...
	/**Constructor3, Create an empty sparse matrix by indicating its rank, and length of value[]*/
	public SparseMatrix(int rank, int valLen) {
		// Update matrix's rank
		this.rank = rank;

		// Create empty data structures for this empty matrix
		for(int i=0; i<valLen; i++) {value.add(0.0);}
		for(int i=0; i<rank+1; i++) {rowPtr.add(0);}
		for(int i=0; i<valLen; i++) {colInd.add(0);}
	}
	...
	public int rowPermute(int i, int j) {
		try {
			...
			// Switch [i,i+1], [j,j+1], and left the other sections to be not updated
			// (Utilizing ArrayList's advantage of high efficiency in search-by-index)
	...
	public SparseMatrix Add(SparseMatrix m) {
		assert m.rank == rank;
		SparseMatrix sum = new SparseMatrix(rank, m.value.size());
...
//		}
	...
	public void matrixSetter(int r, int c, double v) {
		...
		// Insert new element(r,c), iff original(r,c) == 0：
		else {
			//Insert into value[]:
	...
	/** Decompose matrix into the form of L, D, U*/
	public void LUDecompose(SparseMatrix L, SparseMatrix D, SparseMatrix U) {
		...
		for(int i=0; i<rank; i++) {
			for(int j=i+1; j<rank; j++) {
				U.matrixSetter(i, j, retrieveElement(i, j));
				L.matrixSetter(j, i, retrieveElement(j, i));
			}
		}
	}
