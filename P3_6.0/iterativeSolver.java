...
public class iterativeSolver {
	...
	public static double[] iterSolver(int estType, int normType, int modelType, Vector paraGuess, double tolerance) 
		...
		while(V(currPara, normType) > Math.pow(10, -7) && increMag(deltaPara, currPara) > tolerance) {
			...
			prevPara = currPara;
			lineConv *= lineConv(prevPara, currPara, normType);

			// Terminate the iterative solver and return 0 as an indicator of fail to converge when too many iterations
			if(count > 20000) {
				double convResult[] = {0, quadConv(preprevPara, prevPara, currPara, normType), lineConv, V(currPara, normType), count};
				return convResult;
			}
		}

		// Return the result
		double convResult[] = {1, quadConv(preprevPara, prevPara, currPara, normType), lineConv, V(currPara, normType), count};
		return convResult;
	}
