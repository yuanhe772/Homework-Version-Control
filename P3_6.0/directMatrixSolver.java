 ...
 * 
 * Project 3, Task 1
 * The direct matrix solver, to solve x out of Ax - V = 0
 * In project 3, the Power Law problem could be transformed into
 * log(y) = log(c) + m * log(x), therefore linear regression could
 * be used to render the solution for c and m without having to 
...
public class directMatrixSolver {
	...
	public static Vector directSolver(int modelType){
		...
		// Change the model type into "modelType", to properly use loss function of iterativeSolver.V()
