...
public class gradientEsti {
	...
	public static Vector delQN(Vector para) {
		...
		for(int i=0; i<para.len; i++) {
			...
			delVQN.v[i] = (iterativeSolver.V(onePerturbed1) - iterativeSolver.V(onePerturbed2)) / (allPerturbed.v[i]*2);
		}
		return delVQN;
	}
	...
	public static FullMatrix hessianQN(Vector para) {
		...
		for(int i=0; i<para.len; i++) {
			for(int j=0; j<para.len; j++) {
				...
				HessVQN.full[i][j] = (iterativeSolver.V(twoPerturbed1) - iterativeSolver.V(twoPerturbed2) 
						- iterativeSolver.V(twoPerturbed3) + iterativeSolver.V(twoPerturbed4)) /
	 ...
	 * Parameters: Vector para = [Is, kapa, Vth]
	 * Return: Vector delVQN*/
	public static Vector delSC(Vector para) {
		// Perturb the parameter with scalar = perturb
		Vector allPerturbed = para.scale(perturb);
		...
		// The numerator
		double numerator = iterativeSolver.V(perturbed1) - iterativeSolver.V(perturbed2);
	 ...
	 * Parameters: Vector para = [Is, kapa, Vth]
	 * Return: FullMatrix HessVQN*/
	public static FullMatrix hessianSC(Vector para) {
		// Perturb the parameter with scalar = perturb
		Vector allPerturbed = para.scale(perturb);
		...
		// The numerator
		double numerator = iterativeSolver.V(perturbed1) - iterativeSolver.V(para) + iterativeSolver.V(perturbed2);
