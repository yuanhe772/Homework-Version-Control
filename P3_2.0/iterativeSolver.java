...
public class iterativeSolver {
	 ...
	 * Return: double */
	public static double V(Vector para) {
	 ...
	 * Return: None
	 * @throws Exception */
	public static void iterSolver(int estType, int norm, Vector paraGuess, double tolerance) throws Exception {
		...
		while(V(currPara) > Math.pow(10, -7) && increMag(deltaPara, currPara) > tolerance) {
			count += 1;

			// Assign delV and hessV according to different gradient estimation type
			if(estType == QN) {
				delV = gradientEsti.delQN(currPara);
				hessV = gradientEsti.hessianQN(currPara);
			}
			else if(estType == SC) {
				delV = gradientEsti.delSC(currPara);
				hessV = gradientEsti.hessianSC(currPara);
			}
			...
			t = 1;
			norm1 = V(currPara.add(deltaPara, t));
			norm2 = V(currPara.add(deltaPara, t/2));
			while(norm1 > norm2) {
				norm1 = norm2;
				t/=2;
				norm2 = V(currPara.add(deltaPara, t/2));
			...
			System.out.println(count + "th iteration:\n    [Is, Kapa, Vth] = " + Arrays.toString(currPara.v) + 
					...
					"\n    || V || = " + V(currPara) + 
					";  Quadratic convergence's (V(k) - V(0)) / V(k-1)^2 = "+ ((V(currPara) - V(paraGuess))/(prevV*prevV))+"\n");
	...
	public static Vector paraSensi(Vector deltaPara, Vector para) {
		...
		for(int i=0; i<para.len; i++) {
			...
			numerator = iterativeSolver.V(perturbed) - iterativeSolver.V(para) / iterativeSolver.V(para);
