...
public class iterativeSolver {
	 ...
	 * Return: double[] convResult = new double [5];
	 * 								 convResult[0] = Iterative solver's termination indicator(1 for not terminated, 0 for terminated)
	 * 								 convResult[1] = V's quadratic convergence indicator(1 for converged, 0 for fail to converge)
	 * 								 convResult[2] = V's linear convergence indicator
	 * 								 convResult[3] = The final loss function's value
	 * 								 convResult[4] = How many iterations it took for converging
	 * 		[termination indicator(1 for terminated, 0 for not terminated),  quadratic convergence, V's value, count]
		...
		while(V(currPara, normType) > Math.pow(10, -7) && increMag(deltaPara, currPara) > tolerance) {
			...
			if(printEnable == true) {
				System.out.println("The "+count + "th iteration of model "+(MODEL==POW ? "Power Law ":"EKV ") + "with "+
						...
						" method:\n    "+(MODEL==POW ? "[c, m] = " : "[Is, Kapa, Vth] = ") + Arrays.toString(currPara.v)  + 
			...
			if(count > 5000) {
				double convResult[] = {0, quadConv(preprevPara, prevPara, currPara, normType), (lineConv/count < 0.1) ? 1:0, V(currPara, normType), count};
				return convResult;
			}
		}

		// Return the result
		double convResult[] = {1, quadConv(preprevPara, prevPara, currPara, normType), (lineConv/count < 0.1) ? 1:0, V(currPara, normType), count};
		return convResult;
	}
	...
	/**Function: check in each iteration, if V is linearly converging to 0
	...
	public static int lineConv(Vector prevPara, Vector currPara, int normType) throws Exception {
		int convRes = V(currPara, normType) < 1.0001*V(prevPara, normType)? 1 : 0;
		return convRes;
	}

	/**Function: disable or enable the print-outs in iterative solver's inner-loop
	 * Parameters: boolean en
	 * Return: None*/
	public static void printEnable(boolean en) {
		printEnable = en;
	}
}
