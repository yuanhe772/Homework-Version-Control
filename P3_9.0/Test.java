...
public class Test {
	...
	/**Function: test the correctness of direct matrix solver
	 * Parameters: None
	 * Return: None*/
	public static void testDirectSolver(FullMatrix A, Vector V) {
		...
		// Solve the matrix:
		directMatrixSolver.Diagnal(A, V);
		directMatrixSolver.backSub(A,V,X);
		...
		if(A.rank == 2) {
			dataIO.output("\n	"+ (error < Math.pow(10, -7)? "PASSED" : "FAILED") +
					": Direct matrix solver's correctness test! By testing with the ill-conditioned matrix in Note4, "
					+ "slide 50 to see if || Ax-b || < 10^-7");
		}
		else{
			dataIO.output("\n	"+ (error < Math.pow(10, -7)? "PASSED" : "FAILED") +
					": Direct matrix solver's correctness test! By testing with a rank-up-to-4 matrix to see if || Ax-b || < 10^-7");
		}
	}
